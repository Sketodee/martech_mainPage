        ...
        <div className="grid grid-cols-2 gap-1">
          {languageOptions.map(({ language: langName, code }) => (
            <button
              key={code}
              onClick={() => handleLanguageChange(code)}
              className={`relative px-3 py-3 rounded-xl text-sm font-medium transition-all duration-300 ${language === code
  ...
  return (
    ...
    <nav className="z-50 md:py-3 ">
      <div className="w-full lg:w-[80%] xl:w-[70%] mx-auto flex items-center justify-between bg-white  text-black border-1 border-gray-200  p-2 md:p-4 rounded-full shadow-lg shadow-gray-300">
        ...
        {/* Desktop Navigation */}
        <div className="hidden md:flex items-center space-x-4 lg:space-x-8 border-1 border-purple-300 px-4 py-2 rounded-full">
      ...
      <div className={`md:hidden transition-all duration-300 ease-in-out ${isMenuOpen
        ...
        <div className="px-4 py-6 space-y-4 bg-[#29246b] backdrop-blur-sm rounded-lg mt-4 text-white">
